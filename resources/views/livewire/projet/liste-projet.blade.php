                    ...
                    <div class="col-sm-12">
                        <div class="">
                                        ...
                                        <div class="card-body">
                                            <div class="row g-3">
                                                ...
                                                <div class="col-xxl-2 col-sm-3">
                                                    <a href="{{route('create-projet')}}" class="btn btn-soft-success mx-1 w-100" ><i class="mdi mdi-plus-outline align-middle"></i> Ajouter</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="table-responsive">
                                <table style="width: 100%; font-size: 12px;" class="table project-list-table table-nowrap align-middle table-borderless">
                                    <thead>
                                        <tr>
                                        ...
                                        @foreach ($projets as $projet)
                                            <tr>
                                                ...
                                                @elseif ($projet->status == 'Retard')
                                                    <td><button class="btn btn-soft-danger btn-sm btn-rounded">En Retard</button></td>
                                                @elseif ($projet->status == 'Avance')
                                                    <td><button class="btn btn-soft-info btn-sm btn-rounded">Avancer</button></td>
                                                @elseif ($projet->status == 'Suspendu')
                                                    <td><button class="btn btn-soft-dark btn-sm btn-rounded">Suspendu</button></td>
                                                    @elseif ($projet->status == 'activer')
                                                    <td><button class="btn btn-soft-primary btn-sm btn-rounded">En Cours</button></td>
                                                @endif
                                                <td>
                                                ...
                                                <td>
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end">
                                                            <a class="dropdown-item" wire:click="edite('{{ $projet->code }}')">Edit</a>
                                                            @if ($projet->status == 'Suspendu')
                                                                <a class="dropdown-item" wire:click="confirmation('{{ $projet->code }}')">Activer le project</a>
                                                            @else
                                                                <a class="dropdown-item" wire:click="confirmation('{{ $projet->code }}')">Suspendre le project</a>
                                                            @endif
                                                            <a class="dropdown-item" wire:click="confirmationDelete('{{ $projet->code }}')">Delete</a>
                                                            <a class="dropdown-item" href="{{ route('Apercu-projet', ['slug' => $projet->code]) }}">View details</a>
                                                     </div>
                                                    </div>
                                                </td>
                                            </tr>
                ...
                <div class="modal-body">
                    <form wire:submit.prevent='update'>
                        <div class="form-group mb-3">
                            <label for="Projetname" class="col-form-label">Projet Name<span class="text-danger">*</span></label>
                            <div class="col-sm-12">
                                <input wire:model='nom' id="Projetname" name="Projetname" type="text" class="form-control validate" placeholder="Enter Projet Name..." required>
                                @error('nom')
                        ...
                        <div class="form-group mb-3">
                            ...
                            <ul class="list-unstyled user-list validate" id="taskassignee">
                                @foreach ($membre as $user)
                                    <li>
                                        <div class="form-check form-check-primary mb-2 d-flex align-items-center">
                                            <input wire:model.live="usersnewMembres" class="form-check-input" type="checkbox" id="usersnewMembres-{{ $user->user->id }}" name="usersnewMembres[]" value="{{ $user->user->id }}" checked>
                                            <label class="form-check-label ms-2" for="member-{{ $user->user->id }}">{{ $user->user->name }}</label>
                                            <img src="{{ asset($user->user->profile_photo_path ? $user->user->profile_photo_path : 'assets/images/users/avatar-1.jpg') }}" class="rounded-circle avatar-xs m-1" alt="">
                                            <span class="badge m-2 rounded-1 badge-soft-secondary">Membre existant</span>
                                        </div>
                                    </li>
                                @endforeach
                                @foreach ($newmembre as $user)
                                    <li>
                                        <div class="form-check form-check-primary mb-2 d-flex align-items-center">
                                            <input wire:model.live="usersnewMembres" class="form-check-input" type="checkbox" id="usersnewMembres-{{ $user->id }}" name="usersnewMembres[]" value="{{ $user->id }}">
